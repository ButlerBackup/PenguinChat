...
function AnnounceDrop($RoomNumber,$TmpId,$DontDelete='0')
	...
	if ($DontDelete == '0')		// By doing this I can drop them from one move and move them to another without actually dropping them
		{$sql = "DELETE FROM chat_ChatLiveUsers WHERE TmpId = '$TmpId'";
	   	$result = mysql_query ($sql) or Dienice('&e=19', $sql);
   		$sql = "UPDATE chat_ChatUsers SET TmpId = '',TotalTime=TotalTime + ($now - LastAccess) WHERE TmpId = '$TmpId'";
   		$result = mysql_query ($sql) or Dienice('&e=19', $sql);
   		}
...
$sql = 'SELECT TmpId,RoomId FROM ' . chat_ . 'ChatLiveUsers WHERE LastCheckIn < "' . ($now - $TimeOut) . '"';
$result = mysql_query ($sql) or Dienice('&e=19', $sql);
