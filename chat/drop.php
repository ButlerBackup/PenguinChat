...
if ($dropid != $id)
	{$sql = "SELECT UserId FROM chat_ChatUsers WHERE TmpId='$id' AND IsModerator = '1' AND Password = \"$Password\"";
	$result = mysql_query ($sql) or Dienice('&e=19', $sql);
	if (mysql_num_rows($result) == 0)
		{Dienice('&e=22','You do not have permission to drop this person');}
	}
else	{$sql = "SELECT TmpId FROM chat_ChatLiveUsers WHERE TmpId='$id' AND Pass='$k'";
	$result = mysql_query ($sql) or Dienice('&e=19', $sql);
	if (mysql_num_rows($result) == 0)
		{Dienice('&e=22','You do not have permission to drop this person');}
	}
