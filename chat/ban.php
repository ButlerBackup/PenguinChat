...
if ($banid != $id) 	{$sql = "SELECT UserId,TmpId FROM chat_ChatUsers WHERE TmpId='$id' AND IsModerator = '1' AND Password = '$Password'";}
...
$sql = "SELECT * FROM chat_ChatUsers WHERE TmpId = '$banid' LIMIT 1";
$result = mysql_query ($sql) or Dienice('&e=19', $sql);
if (mysql_num_rows($result) == 0)	// They are a guest
	...
	$sql = "SELECT * FROM chat_ChatLiveUsers WHERE TmpId = '$banid' LIMIT 1";
	$result = mysql_query ($sql) or Dienice('&e=19', $sql);
	$BannedUser = mysql_fetch_array($result);
	mysql_free_result($result);

	$sql = "INSERT INTO chat_ChatUsers SET Name='$BannedUser[Name]',TmpId = '$banid',LastAccess='$now',RegDate='$now',Ip='$REMOTE_ADDR',IsBanned = '1'";
	}

else 	{$sql = "UPDATE chat_ChatUsers SET IsBanned = '1' WHERE TmpId = '$banid' LIMIT 1";}
$result = mysql_query ($sql) or Dienice('&e=19', $sql);
