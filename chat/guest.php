...
#$sql = "SELECT COUNT(UserId) FROM chat_ChatLiveUsers WHERE IsGuest = '1'";
#$result = mysql_query ($sql) or Dienice('&e=19', $sql);
#$NumberLoggedIn = mysql_fetch_array($result);
#mysql_free_result($result);
...
#$sql = "INSERT INTO chat_ChatLiveUsers SET UserId = '$Item[UserId]',RoomId='0',LastCheckIn='$now',Name='$Item[Name]'";	// Attributes will be set when they join a room
...
#$sql = "LOCK TABLES chat_ChatUsers AS U WRITE,chat_ChatLiveUsers AS L WRITE";
#$result = mysql_query ($sql) or Dienice('&e=19', $sql);

// Get a Guest Account
$sql = "SELECT U.Name,U.UserId,L.UserId AS Id2 FROM chat_ChatUsers AS U LEFT JOIN chat_ChatLiveUsers AS L ON (U.UserId = L.UserId) WHERE U.IsGuest='1'";
$result = mysql_query ($sql) or Dienice('&e=19', $sql);
...
while ($Item = mysql_fetch_array($result))
	...
	if (!IsANumber($Item[Id2]))	
		{
		$sql = "UPDATE chat_ChatUsers SET LastAccess='$now',Ip='$REMOTE_ADDR' WHERE UserId = '$Item[UserId]'";
		$result2 = mysql_query ($sql) or Dienice('&e=19', $sql);
