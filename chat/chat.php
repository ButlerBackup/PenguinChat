...
if ($Attributes != '')		// Skip saying anything if nothing is new
	...
	$sql = "UPDATE chat_ChatLiveUsers SET LastCheckIn='$now',Attributes='$Attributes' WHERE TmpId = '$id' AND Pass='$k'";
	$result = mysql_query ($sql) or Dienice('&e=19', $sql);

	}
else
	{// Update their information
	$sql = "UPDATE chat_ChatLiveUsers SET LastCheckIn='$now' WHERE TmpId = '$id' AND Pass='$k'";
	$result = mysql_query ($sql) or Dienice('&e=19', $sql);
	}
