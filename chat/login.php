...
if ($Item[Low] > 1) {$Tmp[0] = $Item[Low] - 1;$sql = "INSERT INTO chat_ChatLiveUsers SET TmpId = '$Tmp[0]',Pass='$User[Pass]',RoomId='0',LastCheckIn='$now',Name='$Name'";}
else {$Tmp[0] = $Item[High] + 1;$sql = "INSERT INTO chat_ChatLiveUsers SET TmpId = '$Tmp[0]',Pass='$User[Pass]',RoomId='0',LastCheckIn='$now',Name='$Name'";}
$result = mysql_query ($sql) or Dienice('&e=19', $sql);
...
$sql = "UPDATE chat_ChatUsers SET TmpId = '$Tmp[0]', LastAccess='$now',Ip='$REMOTE_ADDR',TimesLoggedIn=TimesLoggedIn+1 WHERE UserId = '$User[UserId]'";
$result = mysql_query ($sql) or Dienice('&e=19', $sql);
...
if (mysql_num_rows($result) == 0) 	{$sql = "INSERT INTO chat_ChatDailyStats SET Year='$Year',Month='$Month',Day='$Day',Guests='1',MaxUsers='$Item[MaxUsers]'";}
else 					{$sql = "UPDATE chat_ChatDailyStats SET Logins=Logins+1,MaxUsers='$Item[MaxUsers]' WHERE Year='$Year' AND Month='$Month' AND Day='$Day'";}
$result = mysql_query ($sql) or Dienice('&e=19', $sql);
